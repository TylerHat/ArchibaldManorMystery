...
def main():
    ...
    # character_id and scene ride along deliberately. The trainer ignores extra
    # columns, and audit.py's coverage check reads them: without them it reports
    # every row as an unknown character and the lopsided-coverage warning, which
    # is one of the more useful checks, silently never fires.
    write("sft.jsonl", sft, ["messages", "character_id", "scene"])
    write("dpo.jsonl", ready, ["messages", "chosen", "rejected", "character_id", "scene"])
