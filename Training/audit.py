...
# "i cannot" on its own was here and was wrong. It is ordinary period English -
# "I cannot get out of this costume", "what business he had there I cannot say" -
# and it flagged eight perfectly good lines while catching nothing real. What
# marks a MODEL refusing is the object of the verb: it cannot help, provide,
# assist, comply. A suspect cannot get the suit off.
REFUSAL_TELLS = [
    "i cannot help", "i cannot assist", "i cannot provide", "i cannot comply",
    "i cannot answer that", "i cannot do that", "i cannot fulfil", "i cannot fulfill",
    "i cannot engage", "i cannot continue",
...
def main():
    ...
    if dpo:
        ...
        # Counting distinct prompt TEXT was wrong for this game and fired on every
        # healthy dataset. Twelve suspects have twelve different prompts by design,
        # and CaseGenerator rewrites the murder room, the cast and every movement
        # list each playthrough, so two harvests of one character differ legitimately.
        # Training across several cases is a feature: it teaches the model to read the
        # schedule off the prompt instead of memorising one evening.
        #
        # What actually hurts is a TEMPLATE edit - a section renamed, added or dropped -
        # because those rows teach the model to answer a prompt it will never see again.
        # The stable skeleton is the ALL-CAPS section headings, so compare those. The
        # murderer legitimately gets two extra sections (YOUR SECRET, THE ONE THING YOU
        # LIE ABOUT) and Blackwood gets YOUR EXPERT FINDING, so a variant that is a
        # superset of the common core is expected. Only a MISSING core section is drift.
        def headings(text):
            out = []
            for ln in text.splitlines():
                m = re.match(r"^([A-Z][A-Z '/,()-]{6,}?)(?::|$| \()", ln.strip())
                if m:
        ...
        sysd = Counter(
            ...
            for r in sft + dpo)
        sigs = Counter()
        for text, n in sysd.items():
            sigs[headings(text)] += n

        core = set.intersection(*(set(k) for k in sigs)) if sigs else set()
        missing = {sig: core - set(sig) for sig in sigs if core - set(sig)}

        if missing:
            problems.append(
                f"{len(missing)} prompt variant(s) are missing a section that every other "
                "prompt has. That is a template edit, not a different case, and those rows "
                "teach the model to answer a prompt it will never be shown. Keep the rows "
                "matching the template you ship.")
            for sig, gone in missing.items():
                print(f"    {sigs[sig]:4d} rows  missing: {', '.join(sorted(gone))}")
        elif len(sigs) > 1:
            print(f"  ok - one template, {len(sigs)} intended variants, "
                  f"{len(sysd)} prompt texts (a character in a case).")
            for sig, n in sigs.most_common():
                extra = sorted(set(sig) - core)
                label = "base" if not extra else "+ " + ", ".join(extra)
                print(f"    {n:4d} rows  {len(sig):2d} sections  {label}")
        else:
            print(f"  ok - one template throughout ({len(sysd)} prompt texts).")
