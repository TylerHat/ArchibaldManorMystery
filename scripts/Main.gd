...
## A box with a mesh, a matching collision shape and a flat colour.
##
## The mansion no longer uses this (ManorBuilder owns all of that geometry now),
## but CrimeScene.gd builds the victim, the weapon, the trace marks, the gap
## where a weapon should be, and the personal items with it, so it stays. Do not
## delete it again without checking CrimeScene.gd first.
##
## Note it creates a fresh StandardMaterial3D per call, which is why the old
## mansion could not batch. Fine for the handful of crime scene props; do not
## build hundreds of anything with it.
...
## The manor is generated by the Manor node's ManorBuilder script, in the editor
## and again at runtime. This reads back what it built, so the rest of Main.gd
## keeps working against room_centers / grid_pos / rooms_node exactly as before.
##
## REPLACES the old _build_mansion(). Running both produced two complete
## overlapping manors: doubled geometry, doubled collision, and nine pairs of
## exactly coplanar floor slabs fighting over the depth buffer.
func _adopt_manor() -> void:
	# Typed via the class_name rather than left as Node: GDScript resolves
	# method calls at compile time, so manor.rebuild() on a bare Node would not
	# even parse. The cast yields null if the node is the wrong type, which the
	# check below catches.
	var manor: ManorBuilder = get_node_or_null("Manor") as ManorBuilder
	if manor == null:
		push_error("Main: no 'Manor' node with ManorBuilder.gd attached was found as a child of Main. See MANOR_BUILDER.md.")
		return
	...
	room_centers.clear()
	grid_pos.clear()
	var data: Dictionary = manor.get_room_data()
	for rname in data:
		room_centers[rname] = data[rname]["center"]
		grid_pos[rname] = data[rname]["grid"]

	front_door_node = manor.front_door_node
	# NPCs and evidence parent to Decor, never to the generated Shell, so a
	# rebuild cannot delete them out from under a running game.
	# Cast required: get_node_or_null() returns Node, and rooms_node is typed
	# Node3D, which is narrower.
	rooms_node = manor.get_node_or_null("Decor") as Node3D
	if rooms_node == null:
		rooms_node = manor
...
func _spawn_npcs() -> void:
	...
	for c in GameManager.active_characters():
		...
		# Loads Models/Suspects/<id>.glb when present, otherwise falls back to
		# the original coloured capsule. bind_model() hooks up idle/walk.
		# See CHARACTER_MODELS.md.
...
## ManorBuilder's wall pass). Returns [] if there's no path or from == to.
...
## arithmetic that ManorBuilder already owns.
